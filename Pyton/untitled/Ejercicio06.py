...
class OpenFile:
    def __init__(self, parent):
        ...
        self.filemenu.add_command(label='Save',command=lambda: self.save(self.parent,self.text))
        self.filemenu.add_command(label='Quit',command=lambda: self.quit(self.parent))
        self.menubar.add_cascade(label='File', menu=self.filemenu)
        self.parent.config(menu=self.menubar)

    def save(self, root, text):
        self.data = text.get('0.0', tkinter.END)
        self.filename = dialog.asksaveasfilename(parent=root,filetypes=[('Text', '*.txt')],title='Save as...')
